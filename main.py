...
from auth_db import add_auth_user, remove_auth_user, is_user_authorized, get_all_user_ids
import aiohttp
import aiofiles
import zipfile
import shutil
...
async def add_auth_user_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if await is_user_authorized(new_user_id):
            await message.reply_text("User ID is already authorized.")
        else:
            await add_auth_user(new_user_id)
            await message.reply_text(f"✅ User ID {new_user_id} added.")
...
async def remove_auth_user_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if not await is_user_authorized(user_id):
            ...
            await message.reply_text(f"✅ User ID {user_id} removed.")
    except (IndexError, ValueError):
        await message.reply_text("❌ Please provide a valid user ID.")
...
async def restart_handler(_, m):
    if not await is_user_authorized(m.chat.id):
